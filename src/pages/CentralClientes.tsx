...
export function CentralClientes() {
  const { supportClients, loadSupportClients, enterSupportSession, resetClientPassword, passwordResetRequests, passwordResetHistory, loadPasswordResetRequests, loadPasswordResetHistory, resolvePasswordResetRequest, updateClientProductMode } = useStore();
  ...
  const changeProductMode = async (client: any, mode: 'full' | 'financeiro') => {
    if (updatingModeId || (client.productMode || client.product_mode) === mode) return;
    setUpdatingModeId(client.id);
    try {
      await updateClientProductMode(client.id, mode);
      toast.success(`Modo '${mode === 'financeiro' ? 'Carnê-Leão' : 'Sistema Completo'}' ativado para ${client.clinicName || client.name}`);
    } catch (error: any) {
      toast.error(error?.message || 'Não foi possível salvar o modo de acesso deste cliente.');
    } finally {
  ...
  return <div className="max-w-6xl mx-auto space-y-6">
    ...
    <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">{clients.map(client => <div key={client.id} className="p-5 border-b border-slate-100 last:border-0 flex flex-col gap-4 md:flex-row md:items-center md:justify-between hover:bg-slate-50 transition-colors"><div className="flex gap-4 min-w-0"><div className="w-11 h-11 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center shrink-0"><Building2 className="w-5 h-5"/></div><div className="min-w-0"><h2 className="font-semibold text-slate-900 truncate">{client.clinicName || client.name || 'Clínica sem nome'}</h2><p className="text-sm text-slate-500">Responsável: {client.name || 'Não informado'} · <strong>Login: @{client.username || 'sem usuário'}</strong></p>{client.phone && <p className="text-xs text-slate-400 mt-1 flex gap-1 items-center"><Phone className="w-3 h-3"/>{client.phone}</p>}</div></div><div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl border border-slate-200 text-xs">
              ...
              <button
                type="button"
                onClick={() => changeProductMode(client, 'financeiro')}
                disabled={updatingModeId === client.id}
                className={`px-2.5 py-1 rounded-lg font-bold text-[11px] transition-all ${
                  (client.productMode || client.product_mode) === 'financeiro' 
            ...
            <div className="flex items-center gap-2">
              <Button variant="outline" onClick={() => { setResetClient(client); setNewPassword(''); }} className="border-slate-300 text-slate-700 h-9 text-xs font-bold"><KeyRound className="w-3.5 h-3.5 mr-1.5"/>Redefinir senha</Button>
              <Button onClick={() => open(client)} disabled={openingId === client.id} className="bg-slate-900 hover:bg-indigo-600 text-white h-9 text-xs font-bold"><LogIn className="w-3.5 h-3.5 mr-1.5"/>{openingId === client.id ? 'Abrindo...' : 'Suporte'}</Button>
            </div>
          </div></div>)}{!clients.length && <div className="p-12 text-center text-slate-500">Nenhum cliente encontrado.</div>}</div>
